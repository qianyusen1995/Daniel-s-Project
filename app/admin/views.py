...
def admin_auth(f):
    ...
    def decorate_function(*args, **kwargs):
        ...
        auths = list(map(lambda x: int(x), auths.splist(",")))
...
def change_filename(filename):
    ...
    file_name = datetime.datetime.now().strftime("%Y%m%d%H%M%S") + str(uuid.uuid4().hex)+file_info[-1]
    return file_name
...
def tag_del(id=None):
    ...
    if movie_count > 0:
        flash("无法删除标签，数据库中有{}类型的电影，请检查！".format(tag.name), "err")
...
def preview_del(id=None):
    movie = Preview.query.filter_by(id=id).first_or_404()
    db.session.delete(movie)
    db.session.commit()
    flash("删除预告成功！", "ok")
...
# 角色列表
@admin.route("/role/list/<int:page>", methods=["POST", "GET"])
@admin_login_req
...
# 删除角色
@admin.route("/role/del/<int:id>", methods=["POST", "GET"])
@admin_login_req
...
def admin_add():
    ...
    if form.validate_on_submit():
        ...
        if admin_count == 1:
            flash("账号名称已存在！", 'err')
            return redirect(url_for('admin.admin_add', form=form))
        admin = Admin(
            name=data["name"],
